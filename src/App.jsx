...
function SuccessScreen() {
  const [status, setStatus] = useState("loading"); // "loading" | "ok" | "pending"

  useEffect(()=>{
    (async()=>{
      // Tenta até 5x com intervalo de 1.5s esperando o webhook gravar is_pro
      for(let i=0; i<5; i++){
        const session = await getSession();
        if(session?.user){
          const {data:prof} = await supabase.from("profiles").select("is_pro").eq("id",session.user.id).maybeSingle();
          if(prof?.is_pro){ setStatus("ok"); return; }
        }
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 28px",fontFamily:"'Inter',system-ui,sans-serif",color:C.text,textAlign:"center"}}>
      <style>{CSS}</style>
      <div style={{fontSize:13,fontWeight:900,letterSpacing:"0.14em",color:C.yellow,textTransform:"uppercase",marginBottom:40}}>RouteMax</div>

      {status==="loading"&&(
        <>
          <div style={{width:40,height:40,borderRadius:"50%",border:`3px solid ${C.card}`,borderTopColor:C.yellow,animation:"spin 0.75s linear infinite",marginBottom:24}}/>
          <div style={{fontSize:15,color:C.sub}}>Confirmando seu pagamento…</div>
        </>
      )}
      ...
      {status==="ok"&&(
        <div style={{animation:"fadeUp 0.5s cubic-bezier(0.16,1,0.3,1)"}}>
          ...
          <div style={{fontSize:26,fontWeight:900,letterSpacing:"-0.5px",marginBottom:10}}>Bem-vindo ao Pro!</div>
          <div style={{fontSize:14,color:C.sub,lineHeight:1.7,marginBottom:36,maxWidth:280}}>Seu acesso foi ativado. Agora você tem todos os recursos Pro disponíveis.</div>
          <button onClick={goHome} style={{background:C.yellow,border:"none",borderRadius:12,padding:"16px 32px",fontSize:15,fontWeight:900,color:"#0A0A0A",cursor:"pointer",fontFamily:"inherit"}}>
            Ir para o app →
          </button>
        </div>
      )}
      ...
      {status==="pending"&&(
        <div style={{animation:"fadeUp 0.5s cubic-bezier(0.16,1,0.3,1)"}}>
          <div style={{fontSize:56,marginBottom:16}}>✅</div>
          <div style={{fontSize:26,fontWeight:900,letterSpacing:"-0.5px",marginBottom:10}}>Pagamento confirmado!</div>
          <div style={{fontSize:14,color:C.sub,lineHeight:1.7,marginBottom:36,maxWidth:280}}>Seu acesso Pro será ativado em instantes. Se não aparecer, saia e entre novamente na sua conta.</div>
          <button onClick={goHome} style={{background:C.yellow,border:"none",borderRadius:12,padding:"16px 32px",fontSize:15,fontWeight:900,color:"#0A0A0A",cursor:"pointer",fontFamily:"inherit"}}>
            Ir para o app →
          </button>
        </div>
